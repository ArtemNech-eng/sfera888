...
export function CreateTopicForm({ zhkName }: Props) {
  ...
  return (
    <form onSubmit={onSubmit} style={{ display: "grid", gap: 14 }}>
      <p style={{ fontSize: 12, color: "var(--z-muted)" }}>ЖК: {zhkName}</p>

      <div>
        <label className="zen-label" htmlFor="topic-cat">Категория *</label>
        <select
          id="topic-cat"
          name="category"
          value={category}
      ...
      <div>
        <label className="zen-label" htmlFor="topic-title">Заголовок *</label>
        <input
          id="topic-title"
          type="text"
          name="title"
      ...
      <div>
        <label className="zen-label" htmlFor="topic-body">Текст</label>
        <textarea
          id="topic-body"
          name="body"
          value={body}
