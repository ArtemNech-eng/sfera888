...
const faqItems = [
  ...
  },
  {
    ...
    a: 'Нет. Тестовый доступ ограничен по количеству заказов. После тестового периода работа продолжается через токенную модель — вы покупаете токены и тратите их на заказы.',
  },
  {
    q: 'Я должен платить процент с объекта?',
    a: 'Нет. После тестового периода вы работаете через токены. Весь заработок по объекту остаётся вам. Платформа не берёт процент с суммы ремонта — вы покупаете токены и тратите 2 токена на каждый заказ.',
  },
  {
    q: 'Обязательно ли загружать паспорт?',
    a: 'Только если вы хотите пройти тестовый старт с 1–2 тестовыми заказами. Верификация — это входной фильтр для тестового периода. Если не хотите верификацию — можете сразу купить пакет токенов и начать работу.',
  },
  {
