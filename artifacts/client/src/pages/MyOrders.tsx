...
export default function MyOrders() {
  ...
  if (!phone) return (
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column", background: "#f4f6fb", fontFamily: "Inter, sans-serif" }}>
      <div style={{ background: "#111827", padding: "14px 20px", paddingTop: "calc(14px + env(safe-area-inset-top,0px))", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, flexShrink: 0 }}>
        <span style={{ fontSize: 15, fontWeight: 700, color: "#fff" }}>Мои заказы</span>
      </div>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ fontSize: 56, marginBottom: 16 }}>🔑</div>
        <div style={{ fontSize: 18, fontWeight: 700, color: "#111827", marginBottom: 8 }}>Вы не вошли</div>
        ...
        <button onClick={() => window.location.href = `${BASE}/`}
          style={{ padding: "12px 28px", background: "#1d4ed8", color: "#fff", border: "none", borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
          На главную
        </button>
      </div>
    ...
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column", background: "#f4f6fb", fontFamily: "Inter, sans-serif", overflow: "hidden" }}>
      ...
      <div style={{ background: "#111827", display: "flex", alignItems: "center", gap: 10, padding: "14px 20px", paddingTop: "calc(14px + env(safe-area-inset-top,0px))", flexShrink: 0 }}>
        <div style={{ width: 28, height: 28, background: "#2563eb", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="12 8 12 12 14 14"/><path d="M3.05 11a9 9 0 1 0 .5-4.5"/><polyline points="3 3 3 8 8 8"/></svg>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#fff" }}>Мои заказы</div>
          <div style={{ fontSize: 11, color: "#6b7280" }}>{formatPhone(phone)}</div>
        </div>
        <button onClick={() => setConfirmLogout(true)}
          style={{ background: "rgba(255,255,255,.1)", border: "none", borderRadius: 8, padding: "6px 10px", color: "#9ca3af", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>
          Выйти
        </button>
      </div>
      ...
      {confirmLogout && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.5)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: "24px 20px", maxWidth: 320, width: "100%", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 17, fontWeight: 700, color: "#111827", marginBottom: 8 }}>Выйти из аккаунта?</div>
            <div style={{ fontSize: 13, color: "#6b7280", marginBottom: 20, lineHeight: 1.5 }}>
              Вы сможете войти снова, открыв смету по ссылке от мастера
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setConfirmLogout(false)}
                style={{ flex: 1, height: 44, background: "#f3f4f6", border: "none", borderRadius: 12, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", color: "#374151" }}>
                Отмена
              </button>
              <button onClick={handleLogout}
                style={{ flex: 1, height: 44, background: "#ef4444", border: "none", borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", color: "#fff" }}>
                Выйти
              </button>
            </div>
          </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 14px" }}>
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: "60px 20px", background: "#fff", borderRadius: 20, boxShadow: "0 2px 12px rgba(0,0,0,.06)" }}>
            <div style={{ fontSize: 56, marginBottom: 16 }}>📋</div>
            <div style={{ fontSize: 16, fontWeight: 700, color: "#111827", marginBottom: 8 }}>Заказов не найдено</div>
            ...
            {items.map(item => {
              ...
              return (
                <a key={item.id} href={`${BASE}/smeta/${item.token}`}
                  ...
                  <div style={{ background: "#fff", borderRadius: 16, padding: "16px 16px", boxShadow: "0 1px 6px rgba(0,0,0,.06)", border: "1px solid #e5e7eb" }}>
