...
const STATUS_MAP: Record<string, { label: string; color: string; bg: string }> = {
  ...
  in_progress: { label: "В работе", color: "#155724", bg: "#ede9fe" },
...
export default function MyOrders() {
  ...
  if (!phone) return (
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column", background: "#F1FBF3", fontFamily: FONT }}>
      <div style={{ background: "#fff", borderBottom: "1.5px solid #D5EDD8", padding: "14px 16px", paddingTop: "calc(14px + env(safe-area-inset-top,0px))", display: "flex", alignItems: "center", gap: 10, flexShrink: 0, boxShadow: "0 1px 8px rgba(33,160,56,.06)" }}>
        ...
        <div style={{ fontSize: 15, fontWeight: 700, color: "#0F2014" }}>Мои заказы</div>
      </div>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ fontSize: 56, marginBottom: 16 }}>🔑</div>
        <div style={{ fontSize: 18, fontWeight: 700, color: "#0F2014", marginBottom: 8 }}>Вы не вошли</div>
        ...
        <button onClick={() => window.location.href = `${BASE}/`}
          style={{ padding: "12px 28px", background: "#21A038", color: "#fff", border: "none", borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: FONT }}>
          На главную
        </button>
      </div>
    ...
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column", background: "#F1FBF3", fontFamily: FONT, overflow: "hidden" }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1.5px solid #D5EDD8", display: "flex", alignItems: "center", gap: 10, padding: "14px 16px", paddingTop: "calc(14px + env(safe-area-inset-top,0px))", flexShrink: 0, boxShadow: "0 1px 8px rgba(33,160,56,.06)" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#0F2014" }}>Мои заказы</div>
          <div style={{ fontSize: 11, color: "#5E7A62" }}>{formatPhone(phone)}</div>
        </div>
        <button onClick={() => setConfirmLogout(true)}
          style={{ background: "#D5EDD8", border: "none", borderRadius: 10, padding: "6px 12px", color: "#1A8130", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: FONT }}>
          Выйти
        </button>
      </div>
      ...
      {confirmLogout && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.5)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: "24px 20px", maxWidth: 320, width: "100%", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 17, fontWeight: 700, color: "#0F2014", marginBottom: 8 }}>Выйти из аккаунта?</div>
            <div style={{ fontSize: 13, color: "#6b7280", marginBottom: 20, lineHeight: 1.5 }}>
              Вы сможете войти снова, открыв смету по ссылке от мастера
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setConfirmLogout(false)}
                style={{ flex: 1, height: 44, background: "#F1FBF3", border: "1.5px solid #D5EDD8", borderRadius: 12, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: FONT, color: "#1A8130" }}>
                Отмена
              </button>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "14px 14px" }}>
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: "48px 20px", background: "#fff", borderRadius: 18, border: "1.5px solid #D5EDD8", boxShadow: "0 2px 12px rgba(33,160,56,.07)" }}>
            <div style={{ fontSize: 48, marginBottom: 14 }}>📋</div>
            <div style={{ fontSize: 16, fontWeight: 700, color: "#0F2014", marginBottom: 8 }}>Заказов не найдено</div>
            <div style={{ fontSize: 13, color: "#5E7A62", lineHeight: 1.5 }}>
            ...
            {items.map(item => {
              ...
              return (
                <a key={item.id} href={`${BASE}/smeta/${item.token}`}
                  ...
                  <div style={{ background: "#fff", borderRadius: 16, padding: "14px 16px", border: "1.5px solid #D5EDD8", boxShadow: "0 1px 6px rgba(33,160,56,.06)" }}>
