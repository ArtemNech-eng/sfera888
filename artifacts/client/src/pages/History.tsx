...
const STATUS_MAP: Record<string, { label: string; color: string; bg: string }> = {
  ...
  in_progress: { label: "В работе", color: "#0F4C45", bg: "#CCFBF1" },
...
export default function History() {
  ...
  if (notFound) return (
    <div style={{ height: "100dvh", display: "flex", alignItems: "center", justifyContent: "center", background: "#F5FAFA", fontFamily: FONT }}>
  ...
  return (
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column", background: "#F5FAFA", fontFamily: FONT, overflow: "hidden" }}>
      {/* Topbar */}
      <div style={{ background: "#fff", borderBottom: "1.5px solid #D0EDEB", display: "flex", alignItems: "center", gap: 10, padding: "14px 16px", paddingTop: "calc(14px + env(safe-area-inset-top,0px))", flexShrink: 0, boxShadow: "0 1px 8px rgba(109,40,217,.06)" }}>
        <div style={{ width: 32, height: 32, background: "linear-gradient(135deg,#0D9488,#3b82f6)", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#0D2B28" }}>История заказов</div>
          {clientPhone && <div style={{ fontSize: 11, color: "#4A6B69" }}>{formatPhone(clientPhone)}</div>}
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "14px 14px" }}>
        {loading ? (
          <div style={{ display: "flex", justifyContent: "center", paddingTop: 60 }}>
            <div style={{ width: 36, height: 36, border: "3px solid #ddd6fe", borderTopColor: "#0D9488", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
            <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
          </div>
        ) : items.length === 0 ? (
          <div style={{ textAlign: "center", padding: "48px 20px", background: "#fff", borderRadius: 18, border: "1.5px solid #D0EDEB", boxShadow: "0 2px 12px rgba(109,40,217,.07)" }}>
            <div style={{ fontSize: 48, marginBottom: 14 }}>📋</div>
            <div style={{ fontSize: 16, fontWeight: 700, color: "#0D2B28", marginBottom: 8 }}>Заказов нет</div>
            <div style={{ fontSize: 13, color: "#4A6B69" }}>История ваших заказов появится здесь</div>
          </div>
        ) : (
              ...
              return (
                <a key={item.id} href={`${BASE}/smeta/${item.token}`}
                  ...
                  <div style={{
                    ...
                    boxShadow: isCurrent ? "0 0 0 2px #0D9488, 0 2px 12px rgba(109,40,217,.12)" : "0 1px 6px rgba(109,40,217,.06)",
                    border: isCurrent ? "none" : "1.5px solid #D0EDEB",
                  }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                      ...
                      <div style={{ textAlign: "right", flexShrink: 0 }}>
                        ...
                        {isCurrent && <div style={{ fontSize: 10, fontWeight: 600, color: "#0D9488", marginTop: 2 }}>Текущий</div>}
                      </div>
                    </div>
