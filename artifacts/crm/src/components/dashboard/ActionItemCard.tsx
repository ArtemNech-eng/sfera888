    ...
    return (
      <div className={`w-full rounded-lg border border-l-4 ${effectiveLeftBorder} bg-white hover:shadow-sm transition-all ${item.selected ? "ring-2 ring-violet-400 ring-offset-1" : ""} ${item.focused ? "ring-2 ring-violet-500 ring-offset-2 shadow-md" : ""} ${burning ? "bg-red-50/30" : ""}`}
        ...
        <div className="flex items-center gap-2 px-2 py-1.5">
          ...
          {item.amountAtRisk != null && Number(item.amountAtRisk) > 0 && <span className="text-[10px] font-semibold text-red-700 shrink-0">{Number(item.amountAtRisk).toLocaleString("ru-RU")} ₽</span>}
      ...
      <div className="flex items-start gap-2 p-3">
        ...
        <button onClick={() => onOpen(item.id)} className="flex-1 text-left min-w-0">
          ...
          <div className="flex items-center gap-3 mt-1.5 flex-wrap">
            ...
            {item.amountAtRisk != null && Number(item.amountAtRisk) > 0 && <div className="text-xs font-semibold text-red-700">Под риском: {Number(item.amountAtRisk).toLocaleString("ru-RU")} ₽</div>}
          </div>
        </button>
