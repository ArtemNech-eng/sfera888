              ...
              <div key={lead.id} onClick={() => onSelectLead(lead)} className={`bg-card rounded-2xl border border-border/50 shadow-sm overflow-hidden cursor-pointer active:scale-[0.99] transition-transform ${isActive ? "" : "opacity-80"} ${isSelected ? "ring-2 ring-primary/30" : ""}`}>
                <div className="px-4 pt-3 pb-2">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <p className="text-xs text-muted-foreground">{serviceLabel} · {totalArea2} м²</p>
                    {sourceName && <span className="text-[10px] bg-slate-100 text-slate-600 rounded-full px-1.5 py-0.5">{sourceName}</span>}
                  </div>
                </div>
                <div className="flex border-t border-border/50 divide-x divide-border/50" onClick={e => e.stopPropagation()}>
                  ...
                  {lead.status === "sent_to_work" && lead.orderId && <button onClick={() => onOpenOrder(lead.orderId!)} className="flex-1 flex items-center justify-center gap-1 py-2.5 text-emerald-700 hover:bg-emerald-50 text-xs font-medium"><ExternalLink className="w-3.5 h-3.5" />Заказ #{lead.id}</button>}
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-border/50">
                {loading ? (
                  ...
                  <tr><td colSpan={10} className="px-4 py-12 text-center text-muted-foreground">{search || statusFilter || sourceFilter || dateFilter !== "all" ? "Ничего не найдено" : "Новых заявок нет"}</td></tr>
                ) : leads.map(lead => {
                  const srvs = lead.services;
                  const estimate = srvs ? srvs.reduce((sum, s) => sum + s.area * (s.pricePerM2 || 0), 0) : 0;
                  const sourceName = SOURCE_OPTIONS.find(o => o.value === lead.source)?.label;
                  ...
                  return (
                    <tr key={lead.id} onClick={() => onSelectLead(lead)} className={`cursor-pointer hover:bg-slate-50 transition-colors ${isActive ? "" : "opacity-75"} ${isSelected ? "bg-primary/5" : ""}`}>
                      ...
                      <td className="px-3 py-2.5 whitespace-nowrap">
                        {age && <div className={`flex items-center gap-0.5 text-[10px] font-medium ${age.urgent ? "text-red-500" : age.warning ? "text-orange-500" : "text-muted-foreground"}`}><Clock className="w-2.5 h-2.5" />{age.label}</div>}
                      ...
                      <td className="px-3 py-2.5 whitespace-nowrap">
                        {sourceName ? <span className="text-[10px] bg-slate-100 text-slate-600 rounded-full px-2 py-0.5">{sourceName}</span> : <span className="text-[10px] text-muted-foreground/40">—</span>}
                      </td>
                      ...
                      <td className="px-3 py-2.5 pr-4">
                        <div className="flex items-center justify-end gap-1.5 flex-wrap">
                          ...
                          {lead.status === "sent_to_work" && lead.orderId && (
                            <button onClick={e => { e.stopPropagation(); onOpenOrder(lead.orderId!); }} className="inline-flex items-center gap-1 text-[10px] bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-full px-2 py-0.5 font-medium hover:bg-emerald-100 transition-colors"><ExternalLink className="w-2.5 h-2.5" />Заказ #{lead.id}</button>
                          )}
