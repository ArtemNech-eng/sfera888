...
/**
 * Текст, который видит заблокированный мастер при попытке откликнуться.
 * Краткий вариант — для answerCallback / тоста, длинный — для отдельного сообщения.
 */
export const BLOCKED_REJECTION_SHORT =
  "⛔ Доступ к новым заявкам приостановлен. Свяжитесь с оператором.";

export function getBlockedRejectionMessage(orderId: number, _consecutiveCount: number): string {
  return (
    `⛔ <b>Заявка #${orderId} вам не назначена</b>\n\n` +
    `Причина: у вас 2 подряд отменённых заказа, и доступ к новым заявкам временно приостановлен.\n\n` +
    `Вы продолжаете видеть все новые заявки в общем потоке, но мы пока не назначаем вам заказы. ` +
    `Чтобы возобновить работу — свяжитесь с оператором: после разблокировки счётчик обнулится, и вы снова сможете брать заявки.`
  );
}
