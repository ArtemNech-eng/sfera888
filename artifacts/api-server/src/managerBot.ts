...
▸ CRM (crm.sfera-master.ru) — веб-интерфейс для операторов. Управление заявками, заказами, мастерами, финансами, воронкой продаж.

▸ Клиентское PWA (sfera-master.ru/client/) — приложение для заказчиков. Клиент оставляет заявку, видит статус заказа, может чатиться с мастером и оставить отзыв.

▸ Мастер-PWA (sfera-master.ru/master-pwa/) — приложение для исполнителей. Мастер принимает/отклоняет заказы, отправляет смету, фото "до/после", акт выполненных работ.
...
⚠️ ПРАВИЛО КАСКАДА ОШИБОК — КРИТИЧЕСКИ ВАЖНО:
История этого разговора МОЖЕТ содержать ранее неверные данные, сгенерированные без вызова инструмента.
Если руководитель задаёт уточняющий вопрос о числах/ID/суммах, упомянутых ранее в этом чате — ОБЯЗАТЕЛЬНО перепроверь через инструмент прямо сейчас, НЕ опирайся на предыдущие сообщения этого разговора как на источник фактов.
КАЖДЫЙ вопрос о финансах, сметах, заказах — свежий запрос к базе данных, даже если похожие данные уже были в переписке.
...
export async function registerManagerWebhook() {
  ...
  const host = process.env.PUBLIC_HOST ?? "sfera-master.ru";
