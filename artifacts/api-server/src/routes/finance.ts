...
router.post("/transactions/:id/remind", opsAndAdmin, async (req, res) => {
  ...
  const text =
    ...
    `${t.alias ?? "Мастер"}, объекты закрываются поэтапно — это нормально и удобно для всех.\n\n` +
    ...
    `По заказу #${t.order_id} (${serviceType}) ожидается комиссия:\n` +
...
router.post("/masters/:masterId/remind-all", opsAndAdmin, async (req, res) => {
  ...
  const text =
    ...
    `${master.alias}, напоминаем о накопившихся комиссиях.\n\n` +
