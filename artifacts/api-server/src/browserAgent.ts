...
9. CAPTCHA / БЛОКИРОВКА IP:
   - Если видишь "Я не робот", reCAPTCHA, "подозрительный трафик", "проблема с IP", "Доступ ограничен" —
     action: "done", params: { result: "⛔ Сайт заблокировал доступ: обнаружен автоматический браузер (reCAPTCHA / IP-блок). Avito и подобные сайты блокируют запросы с серверов. Попробуй другой сайт или выполни авторизацию вручную." }
   - НЕ проси пользователя ввести данные — капчу нельзя решить через чат.

10. Задача выполнена → done: { result: "описание результата" }`;
