...
async function process() {
  ...
  for await (const line of rl) {
    ...
    if (!valuesMatch) {
      console.log('No VALUES match for line:', line.substring(0, 100));
      continue;
    }
    ...
    for (const tuple of tuples) {
      ...
      if (fields.length < maxChatIdIndex + 1) {
        console.log(`Skipped tuple with ${fields.length} fields`);
        continue;
      }
      total++;
  ...
  console.log(`Total tuples: ${total}, with max_chat_id: ${found}`);
