...
export const citiesTable = pgTable("cities", {
  ...
  /**
   * Город операционно запущен: есть мастера, работает полноценный опыт
   * маркетплейса — каталог мастеров, обычная форма заявки («мастер перезвонит»)
   * и длинный хвост страниц «услуга×город».
   *
   * Города с `isGeoCovered=true, isLaunched=false` находятся в режиме «пре-лонч»:
   * их хаб-/контентные страницы индексируются и копят SEO-вес, но полноценный
   * приём заявок и каталог мастеров не открыты (CTA — лист ожидания + набор
   * мастеров). Флаг переключается по факту масштабирования города.
   *
   * Additive, safe default. На старте запущен только Краснодар (проставляется
   * идемпотентно при загрузке api-server, см. runtime-fix в index.ts).
   */
